describe('number operators', () => {
  ...
  it('- with string without spacing BEFORE - fails', () => {
    const test = () => {
      xEval(doc, null, "asdf- asdf", XPathResult.NUMBER_TYPE);
    };
    assert.throw(test);
  });
  ...
  it('mod without spacing after mod and a string fails', () => {
    const test = () => {
      xEval(doc, null, "'1' mod/html'", XPathResult.NUMBER_TYPE);
    };
    assert.throw(test);
  });
